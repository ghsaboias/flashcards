    ...
    <div className={`container ${isHighIntensityMode ? 'high-intensity' : ''}`}>
      ...
      {isHighIntensityMode && !sessionId && !inReviewMode && (
        <div className="streamlined-start">
          ...
          {/* Table with link logic for the selected set/sets */}
          <div className="section summary" style={{ marginTop: 24 }}>
            {showStats && stats && (
              <div>
                <h3>Stats — {mode === 'set' ? humanizeSetLabel(selectedSet) : getMultiSetLabel()}</h3>
            ...
            {!showStats && ((mode === 'set' && selectedSet) || (mode === 'multi-set' && selectedSets.length > 0)) && (
              <div>
                <button className="btn-tertiary" onClick={viewStats}>
                  View {mode === 'set' ? humanizeSetLabel(selectedSet) : getMultiSetLabel()} Stats
                </button>
              </div>
            )}
          </div>
